...
if __package__ in (None, ""):
    ...
    from hello_llm.permissions import PermissionGate, INTERACTIVE, AUTO_ACCEPT, READ_ONLY  # 权限门（S02）
    ...
    from ..permissions import PermissionGate, INTERACTIVE, AUTO_ACCEPT, READ_ONLY  # 权限门（S02）
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ── S02 权限系统参数（Permission System 最小子集）──
    p.add_argument(
        "--yes", action="store_true",
        help="权限模式 auto-accept：跳过 Y/N 批准，全部工具放行（对照 claude 的 --dangerously-skip-permissions）",
    )
    p.add_argument(
        "--read-only", action="store_true",
        help="权限模式 read-only：写/危险工具一律拒绝，仅只读放行（对照 claude plan 模式安全边界）",
    )
...
def _make_gate(args) -> PermissionGate:
    ...
    一、功能作用
        1. 解析权限模式：--read-only > --yes > 默认 interactive；
        2. 注入批准回调：仅交互 REPL（无 -p）且交互模式时，注入
           repl.py 的 Y/N 批准回调（_ask_permission）；
           无头模式（-p）无交互通道 → asker=None（ask 自动拒绝）。

    二、参数
        args （Namespace）命令行参数（--yes / --read-only / -p）

    三、返回
        PermissionGate：带模式的权限门实例。
    """
    if args.read_only:
        ...
        mode = INTERACTIVE  # 默认：write 级 Y/N 批准
    asker = None  # asker：Y/N 批准回调（默认无）
    if args.prompt is None and mode == INTERACTIVE:
        # 仅交互 REPL 注入 Y/N 回调（论文 §5 approval dialog）。
        # _ask_permission 已在模块顶部双模式块导入（脚本/包两种模式
        # 都可用），此处直接使用 —— 函数体内不再出现相对导入。
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # ── S02 权限门构建（论文图1：loop → permission system → tools）──
    gate = _make_gate(args)  # gate：权限门（模式 + REPL 批准回调）

    if args.prompt is not None:
        return run_headless(args, cfg, gate)  # 无头单次
    return run_repl(args, cfg, gate)  # 交互 REPL
