...
from ..permissions import PermissionGate  # 权限门（S02 新增）：execute 前决策
...
def query_loop(
    ...
    permission_gate: Optional[PermissionGate] = None,  # 权限门（S02 新增）；None = 无权限检查（兼容旧测试）
) -> Iterator[dict]:
...
"""
    ...
    for turn in range(1, max_turns + 1):  # turn：当前轮次（1 起）
        ...
        for tc in response.tool_calls:  # tc：单个工具调用
            ...
            # 日志提示：工具触发（任何运行模式都能看到）
            tool_triggered(tc.name, tc.arguments)
            # ── 权限门（S02 新增）：execute 前统一决策（论文图1 数据流
            #    "loop → permission system → tools"）；拒绝则不执行 ──
            if permission_gate is not None and not permission_gate.decide(tc.name, tc.arguments):
                # 权限拒绝：工具不执行，拒绝原因回填模型（模型据此调整行为）
                result_text = (
                    f"错误：权限拒绝（工具 {tc.name} 未获批准，当前模式 {permission_gate.mode}）"
                )
                log_warn(f"权限拒绝：{tc.name} 未获批准（模式 {permission_gate.mode}）")
            else:
                result_text = execute_tool(tc.name, tc.arguments)  # result_text：工具返回的 tool_result
            # 日志提示：工具执行结果（成功摘要 / 失败详情）
