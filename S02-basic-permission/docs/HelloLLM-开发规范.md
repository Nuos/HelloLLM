...
> 用途: 后续模块开发说明文档
> 结构: 教程式打包，每阶段 = 完整项目（Nuos/HelloLLM/S02-basic-permission/hello_llm）

## 一、项目总览

**目标：** 依据论文分析，按步骤重建 Claude Code 架构，功能、模块逐步添加。
当前为 S02 版本：**S01（CLI 入口层 + Agent-Loop）+ Permission System 最小子集（独立模块，§5 deny-first）**，其余组件留待后续版本。

**后续模块路线（论文章节映射）：**
会话持久化（§9）→ MCP/工具扩展（§6）→ 上下文压缩流水线（§7）→ Hook（§5/§6）→ 子 Agent 委派（§8）→ 记忆（§7.2）。

**仓库结构（教程式打包）：** 每个课程阶段是一个完整项目包（功能 + 配置 + 测试 + 文档），
并列放在仓库根下；当前阶段为 `S02-basic-permission/`：

```text
HelloLLM/                            # GitHub: Nuos/HelloLLM
├── README.md                        # 仓库说明（结构树 + 快速开始）
└── S02-basic-permission/            # ★ 当前阶段：完整项目包（S01 + 权限系统）
    ├── hello_llm/                   #   Python 包（见 3.1 分层，含 permissions/）
    ├── tests/                       #   34 项单元测试
    ├── docs/                        #   开发规范文档（本文件）
    ├── pyproject.toml               #   项目配置
    ├── README.md                    #   项目说明
...
```text
HelloLLM/S02-basic-permission/               # 当前阶段根（完整项目包）
├── hello_llm/                         # Python 包
...
4. 依赖方向单向：entrypoints → query / services/api → tools → utils / permissions；utils、permissions 无依赖（可被任何层引用）。
...
4. 验证流程（在阶段目录 `S02-basic-permission/` 下执行）：
   1. `cd S02-basic-permission && .venv/bin/python -m pytest -q` 全绿；
...
2. **分级信任（graduated trust）**：按动作危险度分级 —— 只读（自动放行）→ 写（需批准）→ 危险（拒绝）；
3. **决策与执行分离**：`PermissionGate.check()` 纯决策（无副作用），`decide()` 负责交互与最终放行。

### 10.2 策略分级表（policies.py）
...
1. **集成点**：`query/agent_loop.py` 在 `execute_tool()` 之前调用 `permission_gate.decide()`，
   拒绝则不执行；拒绝原因作为 `tool_result` 回填模型（模型据此调整行为）；
2. **批准回调**：`repl.py` 的 `_ask_permission()` 注入 `PermissionGate.asker`；
   无头模式（`-p`）无交互 → ask 自动拒绝（安全默认）；
3. **日志事件**：权限拒绝输出 `⚠ 权限拒绝：<工具> 未获批准（模式 <mode>）`（logging 层）；
4. **新增工具必做**：在 `policies.py` 的 `TOOL_POLICIES` 中显式声明等级，否则默认拒绝。

## 九、新增模块开发 Checklist

1. 确定模块归属层（entrypoints / query / services/api / tools / utils），放入 `S02-basic-permission/hello_llm/` 对应文件夹；
...
6. 补测试（正常路径 + 边界 + 回归），pytest 全绿；新增工具必须在 policies.py 声明策略等级并补权限测试；
