...
## S02 范围（S02-basic-permission）

论文图1 将系统分解为七个组件：用户、接口、Agent 循环、权限系统、工具、状态与持久化、执行环境。
S02 = S01（CLI 入口层 + Agent-Loop）+ **Permission System 最小子集**（独立模块）：

| 模块 | 对应论文 | 实现 |
|------|----------|------|
| CLI 入口层 | 图1 "Interfaces"（交互式 CLI / 无头 CLI） | `hello_llm/entrypoints/`（cli / repl / headless / render） |
| Agent-Loop | 图1 "Agent Loop"（§4.1 查询流水线、图5 伪代码） | `hello_llm/query/agent_loop.py`（execute 前过权限门） |
...
目录结构参考 claude-code 源码（restored-src/src）分层：`entrypoints / query / services/api / tools / utils / permissions`。

## 权限系统（S02 新增，最小子集）

论文 §5 的 deny-first（拒绝优先）与分级信任（graduated trust）的最小落地，
实现为独立模块 `hello_llm/permissions/`：
...
**集成点**：`query/agent_loop.py` 在 `execute` 之前调用 `permission_gate.decide()`——
对应论文图1 数据流 "loop → permission system → tools"。
...
# S02 权限模式
python3 -m hello_llm --read-only -p "读取文件"           # 只读模式：写/危险工具全拒
python3 -m hello_llm --yes -p "创建文件 x"               # 自动接受：跳过 Y/N 全放行
# 默认 interactive：REPL 里写操作会弹「允许执行？[y/N]」，无头模式无交互→自动拒绝
...
- **滑动窗口**：多轮对话会让上下文无限增长，超过模型窗口会报
  `prompt_too_long`（论文 §4.5 停止条件）。本阶段用 `--max-context`（默认 30000
