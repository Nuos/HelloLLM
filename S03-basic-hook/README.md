...
## S03 范围（S03-basic-hook）

论文图1 将系统分解为七个组件：用户、接口、Agent 循环、权限系统、工具、状态与持久化、执行环境。
S03 = S01（CLI 入口层 + Agent-Loop）+ **hook 机制（工具管理扩展点）**：
...
目录结构参考 claude-code 源码（restored-src/src）分层：`entrypoints / query / services/api / tools / utils / hooks`。
