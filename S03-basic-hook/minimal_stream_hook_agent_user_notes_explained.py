...
def match_hook(rule: HookRule, tool_name: str) -> bool:
    """
    ❌判断“一条 HookRule 是否匹配当前 tool_name”。
    user：应当如此解释，判断 tool_name 是否配HookManager::_rules 给挂住了/勾住了。
      如果挂住/勾住了，就要进行 HookManager::_rules 的审核要求
      挂住/勾住的条件是 在代码运行周期的特定节点进行设置节点、而且 tool_name 在HookManager::_rules中已经注册了。
    user: 
      如果，tool_name 没有遇到代码运行周期的特定节点、没有在HookManager::_rules中注册、通过了HookManager::_rules 的审查
       按顺序判断，是否执行工具。
...
def hook_worker_protect_write(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if p.name in SENSITIVE_NAMES or p.suffix.lower() in SENSITIVE_SUFFIXES:
        # 处理受保护分支 都在 SENSITIVE_SUFFIXES 中
        return {
            "decision": "deny",
            "reason": f"受保护的敏感文件：{rel}",
        }

    # user：代码运行至此，意味着，hooks判断，受保护内容安全，允许 ***LLM建议的写入类型工具*** 通过。
...
def run_hook_worker(name: str) -> int:
    ...
    try:
        ...
        if name == "protect-write":
            # user：处理 条件分支 protect-write ？受保护的写入类操作/工具 进行判断
            result = hook_worker_protect_write(payload)
        elif name == "redact-read":
            # user：处理 条件分支 redact-read  ？ 受保护的读取类操作/工具 进行判断处理
...
def default_hook_manager() -> HookManager:
    ...
    主进程启动“自己”作为独立 Hook subprocess。
     ...
     user：
      该函数用于 注册默认 HookRule/钩子规则，即生成 初始化HookManager::rules成员
    """
    ...
    return HookManager(
        rules=[
            HookRule(
                """
                user：含义是，在 PreToolUse 节点，匹配 matcher 的工具/操作，要进行 command 筛选判断
                    1.1 首先判断，是否属于 matcher
                    1.2 然后判断，是否满足 command 的保护/审核条件。
                  2.1 如果 匹配到了 matcher（即在节点 PreToolUse 处的 所有 注册了的操作/工具，（都需要经过审核判断）
                      如果没有匹配到 matcher，则LLM建议的工具不需要进行审查/安全审查
                  2.2 而且 满足了 已注册工具的安全性要求/审核，则继续进行；
                      如果匹配到了 matcher，但没有满足安全性要求/审查，则同样不会继续往下处理。
                """
                event="PreToolUse",
                matcher="write_file",
                command=(python, me, "--hook-worker", "protect-write"),
            ...
            HookRule(
                '''
  user:
    所有经过 PreToolUse 注册节点位置的 工具（类型代码）
    首先，判断工具/操作是否匹配 matcher
    其次，如果匹配了matcher，则进行安全性/审查
'''
                event="PreToolUse",
                matcher="edit_file",
                command=(python, me, "--hook-worker", "protect-write"),
            ...
            HookRule(
                '''
  user:
    所有经过 PostToolUse 注册节点位置的 工具（类型代码）
    首先，判断工具/操作 是否匹配 matcher
    其次，如果匹配了 matcher，则进行相关的安全性/审查 （具体审查内容 会在run_hook_worker(...) 中继续分类型）

  user：
    command 中的参数都是什么含义？分别如何使用？
'''
                event="PostToolUse",
                matcher="read_file",
    ...
    for turn in range(1, max_turns + 1):
        ...
        for tc in tool_calls:
            ...
            pre = hooks.run_pre_tool(
                '''
user： 含义是，
  tool_name 要经历 PreToolUse 节点的判断/审查：
  是否在 HookManager::_rules 中注册了节点事件
  工具名称是否匹配了 注册的工具类型
  工具的功能/操作（即 arguments、workspace_root） 是否符合 安全/申请要求。
'''
            ...
            if not pre["allow"]:
                ...
                result = hooks.run_post_tool(
                    '''
  user:含义是，
    tool_name 及其执行结果（此处已经执行完成了，但工具执行的结果，需要进一步核查，然后会回填给LLM）
