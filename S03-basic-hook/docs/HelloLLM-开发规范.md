...
> 用途: 后续模块开发说明文档
> 结构: 教程式打包，每阶段 = 完整项目（Nuos/HelloLLM/S03-basic-hook/hello_llm）

## 一、项目总览

**目标：** 依据论文分析，按步骤重建 Claude Code 架构，功能、模块逐步添加。
当前为 S03 版本：**S01（CLI 入口层 + Agent-Loop）+ hook 机制（工具管理扩展点）**，其余组件留待后续版本。
...
并列放在仓库根下；当前阶段为 `S03-basic-hook/`：

```text
HelloLLM/                            # GitHub: Nuos/HelloLLM
├── README.md                        # 仓库说明（结构树 + 快速开始）
└── S03-basic-hook/                  # ★ 当前阶段：完整项目包（S01 + hook 机制）
...
```text
HelloLLM/S03-basic-hook/               # 当前阶段根（完整项目包）
├── hello_llm/                         # Python 包
...
│   │
│   └── hooks/                         七、hook 机制（★ S03 新增，对照 claude-code src/utils/hooks.ts）
│       ├── __init__.py                包入口（聚合导出）
...
4. 验证流程（在阶段目录 `S03-basic-hook/` 下执行）：
   1. `cd S03-basic-hook && .venv/bin/python -m pytest -q` 全绿；
...
1. 确定模块归属层（entrypoints / query / services/api / tools / utils / hooks），放入 `S03-basic-hook/hello_llm/` 对应文件夹；
