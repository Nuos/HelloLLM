...
def run_headless(args: "Namespace", cfg: ModelConfig, hook_manager: Optional["HookManager"] = None) -> int:
...
"""
    ...
    text = render_events(conv, args.max_turns, stream=not args.no_stream, hook_manager=hook_manager)  # text：模型回复全文
    if text is None:
        return 1  # 模型调用失败 / 被中止
    if args.no_stream:
