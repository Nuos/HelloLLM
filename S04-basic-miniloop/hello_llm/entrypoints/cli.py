...
if __package__ in (None, ""):
    ...
    from hello_llm.entrypoints.repl import run_repl, _ask_permission  # 交互 REPL + Y/N 批准回调
    from hello_llm.hooks import HookManager  # S03：构建工具管理中枢（自动加载 hook 规则）
    from hello_llm.permissions import PermissionGate, INTERACTIVE, AUTO_ACCEPT, READ_ONLY  # S04：权限门
    ...
    from ..permissions import PermissionGate, INTERACTIVE, AUTO_ACCEPT, READ_ONLY  # S04：权限门
    from .headless import run_headless  # 无头模式（单次提问，对照 claude -p）
    from .repl import run_repl, _ask_permission  # 交互 REPL + Y/N 批准回调
...
一、功能作用
    注册全部 CLI 参数：提问（-p/--prompt）、模型（--model）、
    API base（--api-base）、API key（--api-key）、超时（--timeout）、
    轮数上限（--max-turns）、上下文窗口（--max-context）、
    --no-stream、权限模式（--yes/--read-only）、--version。
    供 main 解析 argv，也供测试直接构造参数。

二、输入（input）
    无（固定注册参数集）。

三、输出（output）
    argparse 解析器实例；调用其 parse_args 得到命名空间。    """
    ...
    # ── S04 权限系统参数（Permission System 最小子集）──
    p.add_argument(
        "--yes", action="store_true",
        help="权限模式 auto-accept：跳过 Y/N 批准，全部工具放行",
    )
    p.add_argument(
        "--read-only", action="store_true",
        help="权限模式 read-only：写/危险工具一律拒绝，仅只读放行",
    )
...
def _print_config_source(args) -> None:
    """函数：打印 API key 配置来源提示。

一、功能作用
    启动时向用户说明 API key 从哪来（本地配置文件或 --api-key 参数），
    只打印路径不泄露 key 本身，方便用户核对配置是否正确生效。

二、输入（input）
    args：命令行参数（含 --config/--api-key）。
...
def _make_gate(args) -> PermissionGate:
    """函数：构建权限门。

    一、功能作用（算法）
        1. 解析权限模式：--read-only 优先于 --yes，都没有则默认 interactive；
        2. 只有交互 REPL（没带 -p）才注入 Y/N 批准回调：有用户可问才问；
           无头模式（带了 -p）没有交互通道，回调留空，
           权限询问一律按拒绝处理（安全默认）。

    二、输入（input）
        args：命令行参数（--yes / --read-only / -p）。

    三、输出（output）
        配置好模式与批准回调的权限门，交给 Agent-Loop 在工具执行前裁决。
    """
    if args.read_only:
        mode = READ_ONLY  # 只读模式：写/危险全拒
    elif args.yes:
        mode = AUTO_ACCEPT  # 自动接受：全放行
    else:
        mode = INTERACTIVE  # 默认：写级 Y/N 批准
    asker = None
    if args.prompt is None and mode == INTERACTIVE:
        # 仅交互 REPL 注入 Y/N 回调（_ask_permission 已在模块顶部
        # 双模式块导入，脚本/包两种运行模式都可用）
        asker = _ask_permission
    return PermissionGate(mode=mode, asker=asker)
...
一、功能作用
    委托 utils/config.build_model_config 完成合并：
    优先级"命令行显式参数 > 配置文件 > 内置默认值"；
    API key 的唯一本地来源是配置文件。

二、输入（input）
    args：命令行参数（--model/--api-base/--api-key/--timeout/--config）。
...
一、功能作用（算法，论文 §3.4）
    1. 解析命令行参数（build_parser）；
    2. fail-fast 配置校验：API key 缺失 → 打印配置指引并返回 1，不发请求；
    3. 打印配置来源提示（不泄露 key）；
    4. 构建 hook 管理中枢（自动加载规则）与权限门（模式 + REPL 批准回调）；
    5. 分派：带 -p 走无头（headless），否则走交互 REPL（repl）。

二、输入（input）
    argv：命令行参数列表；None 表示取 sys.argv。

三、输出（output）
    进程退出码：0 成功，1 配置错误或模型调用失败。    """
    ...
    # ── 权限门（S04）：构建系统级裁决器（模式 + REPL 批准回调）──
    gate = _make_gate(args)

    if args.prompt is not None:
        return run_headless(args, cfg, hook_manager, gate)  # 无头单次
    return run_repl(args, cfg, hook_manager, gate)  # 交互 REPL
