...
## S05 范围（S05-basic-statepersist）

论文图1 将系统分解为七个组件：用户、接口、Agent 循环、权限系统、工具、状态与持久化、执行环境。
S05 = S03（S01 + hook 机制）+ **State & Persistence 最小实现（会话持久化）**：

| 模块 | 对应论文 | 实现 |
|------|----------|------|
| CLI 入口层 | 图1 "Interfaces"（交互式 CLI / 无头 CLI） | `hello_llm/entrypoints/`（cli / repl / headless / render） |
| Agent-Loop | 图1 "Agent Loop"（§4.1 查询流水线、图5 伪代码） | `hello_llm/query/agent_loop.py`（工具执行前后过 hook） |
| **状态与持久化** | **图1 "State & Persistence"（§9 会话管理）** | **`hello_llm/state/`（store / session）** |
...
目录结构参考 claude-code 源码（restored-src/src）分层：`entrypoints / query / services/api / tools / utils / hooks / state`。
...
论文图1 "State & Persistence"（§9 会话管理）的最小落地，实现为独立模块
`hello_llm/state/`，对齐 claude-code 源码 `utils/sessionStorage.ts` 的
**转录文件（transcript）**设计：

| 文件 | 职责 |
|------|------|
| `store.py` | 会话 JSONL 存储：`create_session`（meta 首行）/ `append_message`（逐条追加）/ `load_session`（逐行回放恢复）/ `list_sessions`；转录文件读取上限 50MB（对齐源码 MAX_TRANSCRIPT_READ_BYTES） |
| `session.py` | 会话名生成（时间戳 `YYYYMMDD_HHMMSS`，带前缀时剔除非法字符）+ 首条消息提炼标题 |

**存储格式**（JSONL，每行一条 JSON）：首行 meta（system 提示 / 模型 / 创建时间），
之后每行一条 OpenAI 格式消息（user / assistant / tool）。坏行跳过不中断恢复。

**使用方式**：
- `python3 -m hello_llm --save-as 项目A` → REPL 每轮对话自动落盘，退出后历史完整
- `python3 -m hello_llm --resume 项目A` → 恢复该项目继续对话
- `python3 -m hello_llm --save-as`（不带名）→ 自动生成时间戳会话名
- `python3 -m hello_llm --list-sessions` → 列出所有历史会话后退出
- 无头模式 `-p "..." --save-as 记录` → 单轮对话保存为会话

会话文件在 `~/.hellollm/sessions/<会话名>.jsonl`（与 API 配置同目录，跨项目可用）。
