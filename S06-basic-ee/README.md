...
S06 = S01（CLI 入口层 + Agent-Loop + Tools）+ **Execution Environment 最小实现（执行环境）**：
...
目录结构参考 claude-code 源码（restored-src/src）分层：`entrypoints / query / services/api / tools / ee / utils`。

## 执行环境（S06 新增，最小实现）

论文图1 "Execution Environment" 的最小落地，实现为独立模块
`hello_llm/ee/`，对照 claude-code 源码 `src/tools/BashTool/` 的
**子进程执行 + 命令语义分类**设计：

| 文件 | 职责 |
|------|------|
| `runner.py` | 命令执行器：`run_command(command, cwd, env, timeout)` 用 subprocess 跑 shell 命令，返回 `CommandResult`（退出码/stdout/stderr/是否超时） |
| `policy.py` | 命令语义分类：`command_semantics()` 判断命令类型（read 只读 / search 检索 / write 写操作 / neutral 中性 / other 其他），对齐源码 BASH_READ/SEARCH/SILENT/NEUTRAL_COMMANDS 命令集 |
| `config.py` | 执行环境配置：工作目录解析（resolve_cwd）、环境变量合并（merge_env）、超时钳制 1~600 秒（clamp_timeout，对齐源码 getMaxTimeoutMs） |

**使用方式**：bash 工具（`tools/bash_tool.py`）注册进工具表后，模型可直接调用
`bash` 命令——执行统一走 `ee.runner`，受 cwd/env/超时约束；命令类型分类
供后续权限/提示系统复用（read 类自动放行、write 类需确认等）。

**设计原则**：执行环境与工具实现解耦——工具只声明"要跑什么命令"，
环境负责"在哪跑、带什么、跑多久"，与论文图1 的 Execution Environment 组件一致。
